...
def enable_flex_attention_dense_causal(model: torch.nn.Module) -> None:
    """
    Patch all HF attention submodules to use PyTorch FlexAttention kernel
    with *dense causal semantics* (BlockMask.causal()).
    
    This uses the fused FlexAttention CUDA kernel introduced in PyTorch 2.5+,
    enabling streaming tile-based computation comparable to FlashAttention 2.

    Supports MHA, MQA, and GQA architectures.
    """

    for name, attn in model.named_modules():
        # Check if this module is an attention layer
        if not (hasattr(attn, "q_proj") and hasattr(attn, "k_proj") 
                and hasattr(attn, "v_proj") and hasattr(attn, "o_proj")):
            continue
        
        # Only patch once
        if getattr(attn, "_orig_forward", None) is None:
            attn._orig_forward = attn.forward

        # Infer query heads
        if hasattr(attn, "num_heads"):
            n_heads_q = int(attn.num_heads)
        elif hasattr(attn, "n_heads"):
            n_heads_q = int(attn.n_heads)
        elif hasattr(model.config, "num_attention_heads"):
            n_heads_q = int(model.config.num_attention_heads)
        else:
            raise RuntimeError(f"Cannot infer num_heads for {name}")

        # Infer key-value heads (for GQA/MQA)
        if hasattr(model.config, "num_key_value_heads"):
            n_heads_kv = int(model.config.num_key_value_heads)
        elif hasattr(attn, "num_key_value_groups"):
            n_heads_kv = n_heads_q // int(attn.num_key_value_groups)
        else:
...
def enable_flex_attention(attn, n_heads_q, n_heads_kv, head_dim=None):
    """
    Monkey-patch attention.forward to use PyTorch FlexAttention.
    Works on torch>=2.6 (BlockMask.causal() removed).
    """

    def causal_block_mask(B, H, T, device):
        """Equivalent to the old BlockMask.causal()"""
        def causal_mask_mod(b, h, q_idx, kv_idx):
            return q_idx >= kv_idx
        return create_block_mask(
            causal_mask_mod, B=B, H=H, Q_LEN=T, KV_LEN=T, device=device
        )
    ...
    def patched_forward(self, hidden_states, attention_mask=None, position_ids=None,
        ...
        # Infer head_dim dynamically if not fixed
        Hd = head_dim or (q.shape[-1] // n_heads_q)

        # Reshape into (B, H, T, Hd)
        q = q.view(B, T, n_heads_q, Hd).transpose(1, 2)
        k = k.view(B, T, n_heads_kv, Hd).transpose(1, 2)
        v = v.view(B, T, n_heads_kv, Hd).transpose(1, 2)

        # Repeat KV heads if MQA/GQA
        if n_heads_kv < n_heads_q:
            repeat_factor = n_heads_q // n_heads_kv
        ...
        # ---- 🔥 FLEX ATTENTION KERNEL CALL ----
        block_mask = causal_block_mask(B, n_heads_q, T, q.device)
        ...
        return out, None  # match HF API

    attn.forward = patched_forward.__get__(attn, type(attn))
    print("[flex] ✅ Enabled PyTorch FlexAttention kernel for all attention layers.")
...
def main():
    ...
    if attn_label == "flex":
        enable_flex_attention_dense_causal(model)
